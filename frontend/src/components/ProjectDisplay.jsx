    ...
    return (
        <div className="max-w-6xl mx-auto p-6 bg-white">
            ...
            <div className="flex flex-col gap-6 md:flex-row justify-between items-center mb-8">
                ...
                <h1 className="text-3xl font-semibold text-gray-900">{data.project_title}</h1>

                <div className="w-full md:w-auto flex md:items-center order-first md:order-none">
                    <button
                        onClick={() => onBackToSearch(null)}
                        className="inline-flex items-center px-3 py-2 text-sm font-medium text-blue-600 hover:text-blue-700 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 mr-4"
                    >
            ...
            {data.objective && (
                <section className="mb-8">
                    <div className="flex items-center mb-3">
                        ...
                        <h2 className="text-xl font-medium text-gray-900">Objective</h2>
                    </div>
            ...
            {data.modules && data.modules.length > 0 && (
                <section className="mb-8">
                    <div className="flex items-center mb-3">
                        ...
                        <h2 className="text-xl font-medium text-gray-900">Modules</h2>
                    </div>
                        ...
                        {Object.entries(data.technology_stack).map(([category, technologies]) => (
                            technologies && technologies.length > 0 && (
                                <div key={category} className="space-y-2">
                                    <h3 className="font-medium text-gray-900 capitalize">{category.replace('_', ' ')}</h3>
                        ...
                        {Object.entries(data.timeline).map(([week, tasks]) => (
                            tasks && tasks.length > 0 && (
                                <div key={week} className="border border-gray-200 rounded-lg p-4">
                                    <h3 className="font-medium text-gray-900 mb-2 capitalize">{week.replace('_', ' ')}</h3>
                                    <div className="space-y-1">
            ...
            {data.HR && (data.HR.total_employees_required || (data.HR.roles && data.HR.roles.length > 0)) && (
                <section className="mb-8">
                    <div className="flex items-center mb-4">
                        ...
                        <h2 className="text-xl font-medium text-gray-900">Human Resources</h2>
                    </div>
                    {data.HR.total_employees_required && (
                        <div className="mb-4">
                            <span className="text-sm text-gray-600">Total Employees Required: </span>
                            <span className="font-medium text-gray-900">{data.HR.total_employees_required}</span>
                        </div>
                    )}
                            ...
                            {data.HR.roles.map((role, index) => (
                                <div key={index} className="border border-gray-200 rounded-lg p-4">
                                    <div className="flex justify-between items-start mb-2">
                                        ...
                                        {role.count && (
                                            <span className="text-sm font-medium text-blue-600">Count: {role.count}</span>
                                        )}
                                    </div>
                                    <div className="space-y-2 text-sm">
                                        {role.experience_required_in_years && (
                                            <div>
                                                ...
                                                <span className="text-gray-900">{role.experience_required_in_years} years</span>
                                            </div>
                                        )}
                                        {role.expected_salary && (
                                            <div>
                                                <span className="text-gray-600">Expected Salary: </span>
                                                <span className="text-gray-900">{currencySymbol}{role.expected_salary}</span>
                                            </div>
                                        )}
            ...
            {data.deliverables && data.deliverables.length > 0 && (
                <section className="mb-8">
                    <div className="flex items-center mb-3">
                        ...
                        <h2 className="text-xl font-medium text-gray-900">Deliverables</h2>
                    </div>
            ...
            {data.steps && data.steps.length > 0 && (
                <section className="mb-8">
                    <div className="flex items-center mb-4">
                        ...
                        <h2 className="text-xl font-medium text-gray-900">Implementation Steps</h2>
                    </div>
                            ...
                            <div key={index} className="border border-gray-200 rounded-lg p-4">
                                <div className="flex items-start justify-between mb-2">
                                    <div className="flex items-center">
                                        <span className="text-sm font-medium text-gray-900 mr-3">Step {index + 1}</span>
                                        {step.type && (
                                    ...
                                    {step.estimated_time_in_days && (
                                        <span className="text-sm text-gray-600">{step.estimated_time_in_days} days</span>
                                    )}
                                </div>
                                {step.description && (
                        ...
                        {data.estimated_pricing && (
                            <div>
                                <div className="flex items-center mb-3">
                                    ...
                                    <h2 className="text-xl font-medium text-gray-900">Estimated Pricing</h2>
                                </div>
                                <div className="text-2xl font-semibold text-blue-600">{currencySymbol}{data.estimated_pricing}</div>
                            </div>
                        )}
                        {data.conclusion && (
                            <div>
                                <h2 className="text-xl font-medium text-gray-900 mb-3">Conclusion</h2>
